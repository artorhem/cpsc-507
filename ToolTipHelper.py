    ...
    def get_html_from_list(ordered_result, style):
        ...
        for item in ordered_result:
            ...
            if value and \
                ...
                message += '<span style=\" ' + style['title'] + '\"><b>' + key + ':</b></span><br>' + '<span style=\"' + \
                           style['content'] + '\">' + value + '</span>' + " <br><br>"
        return message
...
class ToolTipHelperCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        if CURRENT_VERSION:
            ...
            results = self.match_selection(sel)
            print('sel', sel)
            ...
            if results == {}:
                ...
                link = None
                html_tooltip = Utilities.result_format(results,
                                                       self.keyorder,
                                                       link,
                                                       self.style)
                self.show_tooltip_popup(html_tooltip)
    ...
    def get_user_selection(self, sel):
        ...
        self.location = sel
        last_part_region = self.view.word(sel)
        line_region = self.view.line(sel)
        line = self.view.substr(line_region)
        region_end = last_part_region.end()
        region_begin = line[:region_end - line_region.begin()].rfind(' ') + line_region.begin()
        get_word = sublime.Region(region_begin, region_end)
    ...
    def search_in_json(self, search_result, file_path):
        ...
        print('Searching', search_result)
        for key in list(json_data.keys()):
            matches_found = 0
            items = key.split('.')
            if items[-1] not in search_result:
                # false positive
                continue
            for item in items:
    ...
    def get_css_style(self):
        try:
            ...
            style = {'title': 'color: ' + value['title']['color'] + '; text-decoration: ' + value['title'][
                'text-decoration'],
                     'content': 'color: ' + value['content']['color'] + '; text-decoration: ' + value['content'][
                         'text-decoration'],
                     'link': 'color: ' + value['link']['color'] + '; text-decoration: ' + value['link'][
                         'text-decoration']}
        except Exception:
